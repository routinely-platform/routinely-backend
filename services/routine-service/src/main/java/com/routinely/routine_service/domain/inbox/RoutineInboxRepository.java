...
public interface RoutineInboxRepository extends JpaRepository<RoutineInbox, Long> {
     ...
     * {@code idx_ri_status} 부분 인덱스(WHERE status = 'RECEIVED')를 활용한다.
     */
    @Query("SELECT i.id FROM RoutineInbox i WHERE i.status = :status ORDER BY i.id ASC")
