...
> 이상하므로 **`preferredTime` 정각**으로 확정한다. 오프셋 설정도 두지 않는다 — 필요하면 사용자가
> `preferredTime`을 당기면 된다.
>
> `DEADLINE`은 **21:00 고정**이며 그날 안 한 의무 루틴이 있을 때만 보낸다.
> 빈도형 루틴은 **선호 요일에만, 이번 기간 목표를 채웠으면 보내지 않고**, 주/달 마지막 날 미달이면
> `DEADLINE`으로 보낸다(새 타입 없이 재사용). 상세는 `docs/product/policies.md` §8.
